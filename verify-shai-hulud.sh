...
read -r -d '' PACKAGE_LIST << 'PACKAGES_EOF'
02-echo:0.0.7
...
get_package_version() {
    ...
    if [ -f "$pkg_json" ]; then
        grep -m1 '"version"' "$pkg_json" 2>/dev/null | sed 's/.*"version"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/'
    fi
}
...
if [ "$FOUND_COUNT" -eq 0 ] || [ -z "$FOUND_PACKAGES" ]; then
    ...
    # Found malicious packages - output JSON report
    echo "{\"status\":\"INFECTED\",\"hostname\":\"$(json_escape "$HOSTNAME")\",\"serial\":\"$(json_escape "$SERIAL_NUMBER")\",\"scan_date\":\"$SCAN_DATE\",\"count\":${FOUND_COUNT},\"packages\":[$FOUND_PACKAGES]}"
    exit 1
fi
